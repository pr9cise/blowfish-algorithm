S1 = ["d1310ba6", "98dfb5ac", "2ffd72db", "d01adfb7", "b8e1afed",
      ...
      "6e85076a"]
S2 = ["4b7a70e9", "b5b32944", "db75092e", "c4192623", "ad6ea6b0",
      ...
      "db83adf7"]
S3 = ["e93d5a68", "948140f7", "f64c261c", "94692934", "411520f7",
      ...
      "406000e0"]
S4 = ["3a39ce37", "d3faf5cf", "abc27737", "5ac52d1b", "5cb0679e",
      ...
      "3ac372e6"]
def sboxcalculatoins(Text):
    #preparing sboxes
    """
    for i in range (256):
        interimvalue = bin(int(S1[i], 16))[2:]
        if len(interimvalue) < 32:
            n = 32 - len(interimvalue)
            m = ""
            for j in range(n):
                m += "0"
            m += interimvalue
    ...
    for i in range (4):
        ...
        for j in range (8):
            part += Text[i * 8 + j]
        if i == 0:
            valuearray.append(S1[int(part, 2)])
        elif i == 1:
            valuearray.append(S2[int(part, 2)])
        elif i == 2:
            valuearray.append(S3[int(part, 2)])
        elif i == 3:
            valuearray.append(S4[int(part, 2)])
    print(valuearray)
    for i in range (4):
        if len(bin(int(valuearray[i], 16))[2:]) < 32:
            n = 32 - len(bin(int(valuearray[i], 16))[2:])
            m = ""
            for j in range(n):
                m += "0"
            m += bin(int(valuearray[i], 16))[2:]
            valuearray[i] = m
        else:
            valuearray[i] = bin(int(valuearray[i], 16))[2:]
    print(valuearray)
    add1 = (int(valuearray[0], 2) + int(valuearray[1], 2)) % (2**32)
    xor = add1 ^ int(valuearray[2], 2)
    add2 = (xor + int(valuearray[3], 2)) % (2**32)
    if len(bin(add2)[2:]) < 32:
        m = 32 - len(bin(add2)[2:])
        n = ""
        for i in range(m):
            n += "0"
